...
import ApplyLoanView from '@/components/portal/ApplyLoanView';
import BorrowHistoryView from '@/components/portal/BorrowHistoryView';
import ProfileView from '@/components/portal/ProfileView';
import { LogOut, FileText, History, UserCircle, Menu } from 'lucide-react';
...
export default function BorrowerPortalPage() {
  ...
  if (!mounted || !user || user.role !== 'Borrower') return null;
  ...
  const renderView = () => {
    switch (activeTab) {
      case 'Apply': return <ApplyLoanView />;
      case 'History': return <BorrowHistoryView />;
      case 'Profile': return <ProfileView />;
      default: return <ApplyLoanView />;
    }
  };
        ...
        <div className="flex flex-col h-full">
          <div className="h-16 flex items-center px-6 border-b border-[#262626]">
            <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center mr-3">
              <span className="text-black font-bold text-xl leading-none -mt-0.5">L</span>
            </div>
            <h1 className="font-bold text-lg tracking-tight text-white">Borrower Portal</h1>
          </div>
          ...
          <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
            <p className="px-3 text-xs text-gray-500 uppercase tracking-wider font-semibold mb-2">Menu</p>
            {navItems.map((item) => {
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header className="lg:hidden h-16 border-b border-[#262626] bg-[#121212] flex items-center justify-between px-4">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-white">Borrower Portal</span>
          </div>
          <button 
